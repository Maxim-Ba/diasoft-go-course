...
Необходимо:
* создать Dockerfile для каждого из процессов (Календарь, Хранитель, Планировщик);
* собрать образы и проверить их локальный запуск;
* создать docker-compose файл, который запускает PostgreSQL,
Apache Kafka вместе с контейнером с ZooKeeper и все микросервисы проекта;
